...
let
  ...
  sites = [
    ...
    "internal-project-a.local"
    "site1.local"
    "site2.local"
    "client-b-crm.local"
    "client-b-shop-a.local"
    "client-b-shop-b.local"
    "client-b-web.local"
    "client-b-shop-c.local"
    "client-a-admin.local"
    "client-a-sat.local"
    "client-a-wifi.local"
    "client-a-crm.local"
    "client-a-crm-b.local"
    "client-a-admin-b.local"
    "client-a-web-old.local"
    "company-site.local"
    "client-a-web.local"
    "wooduplicate.local"
    "wpduplicate.local"
  ...
  phpmyadminConfig = pkgs.writeText "config.inc.php" ''
    <?php
    $cfg['blowfish_secret'] = 'PhjrO6nof19tO1BRg7NkdKXa/g6zDXhH3K8oaindreE=';
    $i = 0;
    $i++;
  ...
  virtualHosts = lib.listToAttrs (map (site: lib.nameValuePair site (mkPhpVirtualHost site)) sites)
    ...
    # codebase-mcp.local — dev tooling MCP server (institutional knowledge, cognee)
    # Reusable pattern: every WBD project gets its own instance at this URL.
    "codebase-mcp.local" = {
      addSSL = true;
      enableACME = false;
      sslCertificate = "/var/lib/mkcert/local.pem";
      sslCertificateKey = "/var/lib/mkcert/local-key.pem";
      locations."= /mcp" = {
        proxyPass = "http://127.0.0.1:4110/mcp";
        extraConfig = ''
    ...
    # weaver-mcp.local — reserved for v1.4 Weaver product MCP server (port TBD)
    "weaver-mcp.local" = {
      addSSL = true;
      enableACME = false;
      sslCertificate = "/var/lib/mkcert/local.pem";
  ...
  vhostAdd = pkgs.writeShellScriptBin "vhost-add" ''
    set -euo pipefail
    CERT=/var/lib/mkcert/local.pem
    KEY=/var/lib/mkcert/local-key.pem
    CONF_DIR=/var/lib/nginx/sites-enabled
    WEBROOT=/var/www

    usage() {
      echo "Usage: vhost-add <name> [php|react|proxy]"
      echo "  php    — PHP-FPM site  (default)"
      echo "  react  — SPA with index.html fallback"
      echo "  proxy  — reverse proxy (prompts for upstream)"
      exit 1
    }
    ...
    [ -f "$CONF" ] && { echo "Already exists: $CONF"; exit 1; }
    ...
    EOF
        echo "<?php echo '<h1>$NAME</h1>'; phpinfo(); ?>" > $WEBROOT/$NAME/index.php
        chown -R www-data:www-data $WEBROOT/$NAME
        ;;
      react)
        mkdir -p $WEBROOT/$NAME
        cat > "$CONF" <<EOF
    ...
    EOF
        echo "<!DOCTYPE html><html><head><title>$NAME</title></head><body><h1>$NAME</h1></body></html>" > $WEBROOT/$NAME/index.html
        chown -R www-data:www-data $WEBROOT/$NAME
        ;;
      proxy)
        read -rp "Upstream URL (e.g. http://127.0.0.1:3000): " UPSTREAM
        cat > "$CONF" <<EOF
    server {
        listen 80;
        listen 443 ssl;
    ...
    grep -qF "$NAME" /etc/hosts || echo "127.0.0.1 $NAME" >> /etc/hosts

    systemctl reload nginx
    echo "Created: $CONF — nginx reloaded."
  '';

  vhostRemove = pkgs.writeShellScriptBin "vhost-remove" ''
    set -euo pipefail
    [ $# -lt 1 ] && { echo "Usage: vhost-remove <name>"; exit 1; }
    NAME=$1
    CONF=/var/lib/nginx/sites-enabled/$NAME.conf
    [ ! -f "$CONF" ] && { echo "Not found: $CONF"; exit 1; }
    rm "$CONF"
    systemctl reload nginx
    echo "Removed: $CONF — nginx reloaded."
  '';

in
{
  networking.hosts = {
    "127.0.0.1" = sites ++ reactSites ++ [ "phpmyadmin.local" "pgadmin.local" "oracle.local" "weaver-mcp.local" "codebase-mcp.local" ];
  };
  ...
  services.nginx = {
    ...
    appendHttpConfig = ''
      include /var/lib/nginx/sites-enabled/*.conf;
    '';
  };
  ...
  systemd.tmpfiles.rules = [ "d /var/www/localhost 0775 www-data www-data -" ] ++ tmpfilesRules ++
    ...
    [ "d /var/lib/nginx/sites-enabled 0755 root root -" ] ++
  ...
  environment.systemPackages = [ vhostAdd vhostRemove ];
